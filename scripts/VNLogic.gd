...
var _history: Array = [] #track history, acc an array, pop from front when >50, 
...
func go_back() -> void:
	...
	# Pop current, then pop previous to replay it
	_history.pop_back()
	var prev_index: int = _history.pop_back()
	_waiting_for_input = false
	_choice_pending = false
	_index = prev_index
	_process_next()
