...
func _build_ui() -> void:
	...
	bg.color = Color(0.15, 0.52, 0.20, 1.0)
	...
	title.add_theme_color_override("font_color", Color(1.0, 1.0, 0.80, 1.0))
	vbox.add_child(title)

	var sub := Label.new()
	sub.text = "You survived and escaped Chester!"
	sub.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	sub.add_theme_font_size_override("font_size", 28)
	sub.add_theme_color_override("font_color", Color(0.88, 1.0, 0.75, 1.0))
	vbox.add_child(sub)
